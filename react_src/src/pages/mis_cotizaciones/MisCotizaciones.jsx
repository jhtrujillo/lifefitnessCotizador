...
export default function MisCotizaciones() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      {/* Header Premium */}
      <header className="app-header" style={{ background: 'oklch(18% .008 270)', color: 'white', padding: '16px 24px', borderBottom: '3px solid oklch(58% .22 25)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div className="header-actions" style={{ display: 'flex', gap: '12px' }}>
          ...
          <a href="cotizador.html" style={{ background: 'oklch(58% .22 25)', color: 'white', border: 'none', padding: '8px 16px', fontWeight: 600, fontSize: '12px', borderRadius: '4px', textTransform: 'uppercase', fontFamily: 'Oswald, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', textDecoration: 'none' }}>
            ✨ Crear Nueva
          </a>
      ...
      <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '32px 24px', width: '100%', boxSizing: 'border-box', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        <div className="panel-card" style={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', gap: '16px' }}>
            <h2 style={{ margin: 0, fontFamily: 'Oswald, sans-serif', textTransform: 'uppercase', color: 'oklch(20% .005 270)' }}>📂 Historial de Cotizaciones guardadas</h2>
              ...
              <table className="quotes-table">
                ...
                <tbody>
                  {quotesListDisplay.map(q => (
                    <tr key={q.id}>
                      <td style={{ fontWeight: 700, color: 'oklch(30% .01 270)' }}>{q.quote_no}</td>
                      <td style={{ fontWeight: 600 }}>{q.client_name}</td>
                      <td style={{ color: 'oklch(45% .01 270)', fontSize: '13px' }}>{q.adviser}</td>
                      <td style={{ color: 'oklch(58% .22 25)', fontWeight: 700 }}>{q.total_formatted}</td>
                      <td style={{ fontSize: '12px', color: 'oklch(50% .01 270)' }}>{q.updated_at_formatted}</td>
